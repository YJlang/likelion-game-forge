...
export const Route = createFileRoute("/scoreboard")({
  head: () => ({
    meta: [{ title: "실시간 점수판 — LIKELION MT" }, { name: "description", content: "팀별 실시간 점수와 게임 내역" }],
  }),
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid lg:grid-cols-2 gap-5">
        {sorted.map((t, i) => {
          ...
          return (
            ...
            >
              <div className="absolute -top-20 -right-20 w-64 h-64 rounded-full opacity-30 blur-2xl"
                   style={{ background: `var(--${t.colorVar})` }} />
              <div className="relative flex items-center justify-between gap-6">
                <div>
                  <div className="flex items-center gap-3">
                    <span className={`font-display text-5xl ${isFirst ? "text-accent" : "text-muted-foreground"}`}>
                      {isFirst ? "👑" : `${i + 1}위`}
                    </span>
                    <span className="font-display text-4xl">{t.emoji} {t.name}</span>
                  </div>
                  <div className="text-xl mt-1" style={{ color: `var(--${t.colorVar})` }}>{t.leader}</div>
                </div>
                <motion.div
                  key={scores[t.id]}
                  ...
                  style={{ textShadow: isFirst ? "0 0 40px var(--accent)" : undefined, color: isFirst ? "var(--accent)" : undefined }}
                >
                  {scores[t.id] ?? 0}
                </motion.div>
              </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                <button onClick={() => manualAdjust(t.id, +1, "수동 +1")} className="px-3 py-1.5 rounded-lg bg-success/20 text-success text-sm font-bold border border-success/40">+1</button>
                <button onClick={() => manualAdjust(t.id, -1, "수동 -1")} className="px-3 py-1.5 rounded-lg bg-destructive/20 text-destructive text-sm font-bold border border-destructive/40">-1</button>
                <button
                ...
                {expanded === t.id && (
                  ...
                  >
                    <div className="border-t border-border pt-3 space-y-1.5 text-sm max-h-64 overflow-y-auto">
                      {log.filter((e) => e.team === t.id).slice().reverse().map((e) => (
                        <div key={e.id} className="flex justify-between gap-3">
                          <span className="text-muted-foreground">
                            {e.game === "manual" ? "수동" : GAME_LABEL[e.game]} · {e.reason}
                          </span>
                          <span className={`font-display tabular-nums ${e.delta > 0 ? "text-success" : "text-destructive"}`}>
                            {e.delta > 0 ? "+" : ""}{e.delta}
                          </span>
                        </div>
                      ))}
      ...
      <ConfirmModal
        ...
        onConfirm={() => { resetAll(); toast.success("초기화 완료"); setResetOpen(false); }}
      />
    </div>
  );
}
